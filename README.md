...
音频资源会由安装器先探测国内 Gitee 和 [GitHub 资源 Release](https://github.com/hanserdesu/japanese/releases/tag/wcp-jp-resources-v1.0.0)，按可用路由自动下载，并进行完整性校验。Gitee 路由使用小于平台单附件限制的分卷，安装器会自动下载、合并和校验；下载中断时会保留临时缓存，支持下次运行通过 HTTP Range 继续，普通用户只需要下载主 Release 的一键安装包。
