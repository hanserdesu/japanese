...
**正在使用旧版本的用户直接下载 v1.2.9.1 覆盖安装一次即可**，无需先卸载——从 v1.2.7 起安装器支持自更新，之后的新版本会自动提示升级，不再需要手动换包。v1.2.9.1 相比 v1.2.1 的改进（含 v1.2.9 全部改进与性能优化）：（含 v1.2.5/v1.2.6/v1.2.7/v1.2.8 全部改进）：
