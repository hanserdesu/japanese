...
[下载 WCP 日语词书一键安装包 v1.2.9.1](https://github.com/hanserdesu/japanese/releases/download/wcp-jp-v1.2.9.1/WCP-Japanese-OneClick-Installer-wcp-jp-v1.2.9.1.zip)（发布页：[wcp-jp-v1.2.9.1](https://github.com/hanserdesu/japanese/releases/tag/wcp-jp-v1.2.9.1)）。正在使用旧版本的用户直接覆盖安装一次即可；v1.2.7 起安装器自带自更新，之后无需再手动换包。本次 v1.2.9 与多语言版同步收敛「部分机器上单词发音变成英语 AI 语音」：单词音频同步补进游戏原生目录，已安装的用户运行一次游戏即自动补缺，无需重装。
