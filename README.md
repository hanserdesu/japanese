...
1. 下载 [WCP 日语词书一键安装包 v1.2.7](https://github.com/hanserdesu/japanese/releases/download/wcp-jp-v1.2.7/WCP-Japanese-OneClick-Installer-wcp-jp-v1.2.7.zip)（SHA-256 `b03f17188dd48d25caf23fe2a583b625f0af2e0848940bae68957cdc01ccbbf3`，发布页见 [wcp-jp-v1.2.7](https://github.com/hanserdesu/japanese/releases/tag/wcp-jp-v1.2.7)）。
2. 解压安装包。
3. 关闭万词破，双击解压后最外层的 `01_双击运行我.cmd`，不要进入 `support` 文件夹。
4. 等待安装器完成，然后重新启动游戏。安装窗口会一直保留，请点击右上角 X 关闭；按 Enter 不会关闭窗口。

**正在使用旧版本的用户直接下载 v1.2.7 覆盖安装一次即可**，无需先卸载——从 v1.2.7 起安装器支持自更新，之后的新版本会自动提示升级，不再需要手动换包。v1.2.7 相比 v1.2.1 的改进（含 v1.2.5/v1.2.6 全部改进）：
